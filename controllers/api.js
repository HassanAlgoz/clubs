...
module.exports = function(app) {
	...
	app.get('/api/clubs', function(req, res) {
		...
		Club.find({}).sort({date: -1}).exec(function(err, clubs) {
			if (clubs)
				res.json(clubs);
			else
	...
	// Create a club
	app.post('/api/clubs', User.isLoggedIn, function(req, res) {
	...
	// Modify a club
	app.put('/api/clubs/:clubName', function(req, res) {
		...
		// if (req.body.password === passwd) {
			...
			Club.findOne({ name: new RegExp(name, 'i') }, function(err, club) {
				...
				if (club) {
					...
					res.send('could not modify club');
				}
	...
	// Delete a club
	app.delete('/api/clubs/:clubName', function(req, res) {
